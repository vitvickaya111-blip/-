...
async def download_guide_handler(callback: CallbackQuery, repo: RequestsRepo):
    ...
    # Upsell: PDF after quiz
    import asyncio
    from keyboards.inline import get_upsell_paid_pdf_keyboard
    await asyncio.sleep(2)

    upsell_text = """👭 **СПЕЦИАЛЬНОЕ ПРЕДЛОЖЕНИЕ ДЛЯ ТЕБЯ!**

Ты только что получила гайд по Вьетнаму... А что, если я скажу, что у меня есть **ЕЩЕ КРУЧЕ?** 🔥

📖 **"От мечты до посадочного"** — мой главный гайд по релокации!

**30 страниц пошаговой инструкции:**
✅ 10 шагов от мечты до переезда
✅ 15 стран (не только Вьетнам!)
✅ Чек-листы, шаблоны, 50+ ресурсов
✅ Финансовый план

**🎁 + БОНУСЫ:**
• Промокод -10% на консультацию
• Закрытый канал с обновлениями
• Google-таблица для планирования

**Обычная цена:** $50
**Твоя цена прямо сейчас:** $40 (скидка 20%)

⏰ Спецпредложение действует 24 часа!

Это не просто гайд — это твоя дорожная карта к свободе! 🌍"""

    await callback.message.answer(upsell_text, reply_markup=get_upsell_paid_pdf_keyboard(), parse_mode="Markdown")
...
async def get_guide_handler(callback: CallbackQuery, repo: RequestsRepo):
    ...
    # Upsell: Paid PDF after free guide
    import asyncio
    from keyboards.inline import get_upsell_paid_pdf_keyboard
    await asyncio.sleep(3)

    upsell_paid_pdf_text = """💡 **Понравился гайд по Вьетнаму?**

Хочу поделиться с тобой кое-чем важным...

У меня есть **ПОЛНЫЙ гайд по релокации** — "От мечты до посадочного" 📖

**Всё, что нужно знать ДО переезда в одном месте:**

✅ **10 шагов релокации** — пошаговый план действий
✅ **15 стран с детальным анализом** — не только Вьетнам!
✅ **Чек-листы на каждый этап** — ничего не забудешь
✅ **50+ проверенных ресурсов** — сайты, сервисы, контакты
✅ **Финансовый план** — сколько нужно и как накопить
...
**30 страниц конкретики за $50!**

Хочешь переехать — начни с плана. Этот гайд — твой первый шаг! 🌍"""

    await callback.message.answer(upsell_paid_pdf_text, reply_markup=get_upsell_paid_pdf_keyboard(), parse_mode="Markdown")
