...
async def download_guide_handler(callback: CallbackQuery, repo: RequestsRepo):
    ...
    # Upsell: Community after quiz
    import asyncio
    from keyboards.inline import get_upsell_community_keyboard
    await asyncio.sleep(2)

    upsell_text = """👭 **СПЕЦИАЛЬНОЕ ПРЕДЛОЖЕНИЕ!**

Понравился гайд?

Представь, что у тебя есть **целое сообщество** женщин, которые уже живут за границей или активно планируют переезд!

**Закрытое сообщество "Женщины в движении"** - всего $30/месяц

Что внутри:
💬 Поддержка 24/7 от единомышленниц
📅 Еженедельные групповые созвоны со мной
🎓 Мастер-классы от экспертов
👩‍💼 Нетворкинг и полезные знакомства
🌍 Meetup'ы в разных городах

Это больше, чем чат. Это твоя новая семья! 💜"""

    await callback.message.answer(upsell_text, reply_markup=get_upsell_community_keyboard(), parse_mode="Markdown")
...
async def get_guide_handler(callback: CallbackQuery, repo: RequestsRepo):
    ...
    upsell_paid_pdf_text = """💎 **ТЕБЕ ПОНРАВИЛСЯ ГАЙД?**

Это был только старт!

У меня есть **ПОЛНЫЙ ГАЙД ПО РЕЛОКАЦИИ** - комплексный план переезда в любую страну.

📕 **"Полный гайд по релокации"** - $50

Это не просто гайд про Вьетнам. Это:
✅ Сравнение 15+ стран для релокации
✅ Пошаговые инструкции по визам
✅ Реальные бюджеты и калькуляторы
✅ Как найти работу удаленно
✅ Психологическая подготовка
✅ Шаблоны всех документов

**Бонусы:**
🎁 База контактов (врачи, юристы, школы)
🎁 Таблица сравнения стран
🎁 Видео-интервью с женщинами, которые переехали

Это твоя дорожная карта к свободе! 🗺️"""
